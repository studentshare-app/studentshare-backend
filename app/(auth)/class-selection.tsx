  ...
  return (
    <Animated.View style={{ opacity, transform: [{ translateY }, { scale }] }}>
      ...
      >
        ...
        {/* Icon box — coloured per-item like dashboard cards */}
...
export default function ClassSelectionScreen() {
  ...
  function handleBack() {
    if (isSignupMode)    router.replace(ROUTES_SIGNUP)
    else if (isEditMode) router.replace('/(tabs)/profile')
    else                 router.replace('/(auth)/college-selection' as any)
  }
        ...
        ListEmptyComponent={
          <View style={s.emptyBox}>
            ...
            <Text style={s.emptyTitle}>
              {search.length > 0 ? 'No classes found' : 'No classes available'}
            </Text>
            <Text style={s.emptySubtitle}>
              {search.length > 0 ? 'Try a different search term' : 'No classes have been added for this college yet'}
            </Text>
          </View>
        }
          ...
          {saving ? (
            ...
            <View style={s.continueBtnInner}>
              ...
              <View style={[s.continueBtnArrow, !selected && s.continueBtnArrowDisabled]}>
                <Ionicons name={isEditMode || isSignupMode ? 'checkmark' : 'checkmark'} size={16} color={selected ? C.orange : C.textMute} />
              </View>
            </View>
          )}
        </TouchableOpacity>
...
const m = StyleSheet.create({
  ...
  iconWrap: { width: 64, height: 64, borderRadius: 20, overflow: 'hidden', marginBottom: 14, shadowColor: C.orange, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 14, elevation: 6 },
  ...
  saveBtn:  { flex: 1, padding: 14, borderRadius: 14, backgroundColor: C.orange, alignItems: 'center', shadowColor: C.orange, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 5 },
