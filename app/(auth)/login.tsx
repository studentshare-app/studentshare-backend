 ...
 * app/(auth)/login.tsx
 *
 * New visual style — large serif headline, underline inputs, dark near-black bg.
 * Auth logic unchanged: Supabase, PKCE OAuth, sanitiseAuthError, useLoginRateLimit,
 * offline detection, exchangeCodeForSession.
 */
...
// ── Palette (matches the web design source) ───────────────────────────────────
const P = {
  bg:          '#07080C',
  bgCard:      '#10131C',
  border:      '#161B27',
  borderHover: '#E8692A',   // orange accent from web design
  text:        '#EEF0F8',
  muted:       '#6E7A96',
  dimmed:      '#353D52',
  accent:      '#E8692A',
...
export default function LoginScreen() {
  ...
  const { attempt, reset: resetRateLimit, isLocked, getRemainingSeconds } = useLoginRateLimit()
  ...
  async function handleLogin() {
    setError('')
    if (isOffline) { setError('No internet connection. Please connect and try again.'); shake(); return }
    if (!email.trim() || !password) { setError('Please fill in all fields.'); shake(); return }
  ...
  async function handleOAuth(provider: 'google') {
    ...
    try {
      ...
      if (result.type === 'success' && result.url) {
        ...
        if (!exchangeError) { setSocial(null); router.replace(ROUTES.TABS); return }
        const { data: { session } } = await supabase.auth.getSession()
        if (session) { setSocial(null); router.replace(ROUTES.TABS); return }
  ...
  const isFormDisabled = loading || !!socialLoading || isLocked()
    ...
    <View style={[styles.root]}>
      ...
      {isOffline && (
        <View style={[styles.offlineBanner, { top: insets.top + 8 }]}>
          ...
          <Text style={styles.offlineText}>No internet — sign-in requires a connection</Text>
        </View>
      )}
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          >
            <Ionicons name="arrow-back" size={26} color={P.text} />
          </TouchableOpacity>
          ...
          <Animated.View style={[styles.formWrap, { transform: [{ translateX: shakeX }] }]}>
            ...
            {isLocked() && lockCountdown > 0 && (
              <View style={styles.lockRow}>
                ...
                <Text style={styles.lockText}>Locked · try again in {lockCountdown}s</Text>
              </View>
            )}
            ...
            <Pressable
              style={({ pressed }) => [
                styles.googleBtn,
                pressed && !isOffline && styles.googleBtnPressed,
                (!!socialLoading || isOffline) && { opacity: 0.5 },
              ]}
            ...
            >
              {socialLoading === 'google'
                ...
                : <>
                    <Ionicons name="logo-google" size={20} color={isOffline ? P.muted : P.text} />
