...
const ROUTES_SIGNUP = '/(auth)/signup' as const
...
function CollegeCard({
  ...
  return (
    <Animated.View style={{ opacity, transform: [{ translateY }, { scale }] }}>
      ...
      >
        {/* Left orange glow accent when selected */}
  ...
  return (
    <View style={s.root}>
      ...
      <Animated.View style={[s.hero, { paddingTop: insets.top + 12 }, { opacity: heroOpacity, transform: [{ translateY: heroY }] }]}>
        ...
        {/* Back / navigation */}
        {isEditMode ? (
          <TouchableOpacity style={s.backBtn} onPress={() => router.replace('/(tabs)/profile')} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Ionicons name="arrow-back" size={15} color={C.textSub} />
            <Text style={s.backBtnText}>Back to Profile</Text>
          </TouchableOpacity>
        ) : isSignupMode ? (
          <TouchableOpacity style={s.backBtn} onPress={() => router.replace(ROUTES_SIGNUP)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Ionicons name="arrow-back" size={15} color={C.textSub} />
...
const s = StyleSheet.create({
  ...
  // Back button — pill style from index.tsx nav
  ...
  // Hero text — matches index.tsx display style
  ...
  // Offline notice — from index.tsx offline banner style
  ...
  // Search bar — dark surface style matching index.tsx nav search box
  ...
  // Short name pill — mirrors index.tsx TagChip
  ...
  checkGrad:   { flex: 1, justifyContent: 'center', alignItems: 'center', shadowColor: C.orange, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.4, shadowRadius: 8, elevation: 3 },
  ...
  // Selected banner — mirrors index.tsx motCard left border style
  ...
  // Continue button — matches index.tsx primary button
