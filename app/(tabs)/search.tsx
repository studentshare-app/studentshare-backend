...
type PersonResult = {
  ...
  college?: { short_name: string } | null
  is_verified?: boolean
}
...
export default function SearchScreen() {
  ...
  const downloadFile = async (item: Result) => {
    ...
    try {
      ...
      const ext     = item.file_url.split('.').pop()?.split('?')[0] ?? 'pdf'
      const dirInfo = await FileSystem.getInfoAsync(DOWNLOAD_DIR)
      if (!dirInfo.exists) await FileSystem.makeDirectoryAsync(DOWNLOAD_DIR, { intermediates: true })
      const path    = DOWNLOAD_DIR + item.id + '.' + ext
      if (downloadedIds.has(item.id)) {
        const existing = await FileSystem.getInfoAsync(path)
        if (existing.exists) { Alert.alert('Already saved', 'This file is already on your device.'); setDownloading(null); return }
      }
      const res = await FileSystem.downloadAsync(item.file_url, path)
      if (res.status === 200) {
    ...
    } catch (e: any) { Alert.alert('Download failed', e?.message ?? 'Please try again.') }
              ...
              : trendingForum.length === 0
                ...
                : trendingForum.map(post => (
                    <ForumCard
                      key={post.id}
                      post={post}
                      onPress={() => router.push({ pathname: '/forum' as any, params: { post_id: post.id, back: BACK } })}
                    />
                  ))
            }
          </View>
