...
export default function StudyMaterialsScreen() {
  ...
  const downloadMaterial = useCallback(async (item: MaterialRecord) => {
    ...
    try {
      ...
      void supabase.from('material_downloads').insert({ user_id: userId, material_id: item.id }).then(() => {})
      Alert.alert('✓ Added to Library', `"${item.title}" is now in your downloads library.`)
    } catch (e: any) {
      Alert.alert('Download failed', e?.message ?? 'Could not download. Please try again.')
    }
  }, [userId])
