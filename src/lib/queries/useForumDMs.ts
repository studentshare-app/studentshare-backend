...
export function useForumDMs(userId: string | null) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'sq_messages' },
        fetchConversations
      )
      .on('postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'sq_conversations' },
        fetchConversations
      )
      .subscribe()
...
export async function getOrCreateConversation(userA: string, userB: string) {
  const { data, error } = await supabase.rpc('get_or_create_sq_conversation', {
    user_a: userA,
    user_b: userB,
  })
